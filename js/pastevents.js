...
const app = createApp ({//recibe propiedades 
    ...
    created(){ // es un metodo que queremos que se ejecute una sola vez creada la aplicacion
        ...
        .then (data => {
            ...
            this.arrayEventosPasados = this.arrayEventos.events.filter(evento => evento.date < this.arrayEventos.currentDate )
            /* console.log(this.arrayEventosPasados); */
            this.checkboxCategory = this.arrayEventosPasados.map( evento => evento.category)
    ...
    computed: {
        filtro(){
            this.filtrarDoble = this.arrayEventosPasados.filter((tarjeta) => tarjeta.name.toLowerCase().includes(this.buscador.toLowerCase()) && ( this.checked.includes(tarjeta.category) || this.checked.length == 0))  
        }   
    }
})
app.mount ('#app')